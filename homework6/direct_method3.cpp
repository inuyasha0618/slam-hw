...
void DirectPoseEstimationMultiLayer(
    ...
    for (int i = 1; i < pyramids;i++) {
        cv::Mat tmp1, tmp2;
        cv::pyrDown(pyr1.back(), tmp1, cv::Size((int)(pyr1.back().cols * pyramid_scale), (int)(pyr1.back().rows * pyramid_scale)));
        cv::pyrDown(pyr2.back(), tmp2, cv::Size((int)(pyr2.back().cols * pyramid_scale), (int)(pyr2.back().rows * pyramid_scale)));
        pyr1.push_back(tmp1);
        pyr2.push_back(tmp2);
    }
