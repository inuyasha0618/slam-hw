...
void computeAngle(const cv::Mat &image, vector<cv::KeyPoint> &keypoints) {
	...
	// START YOUR CODE HERE (~7 lines)
        ...
        int weighted_x = 0, weighted_y = 0, kp_x = (int)kp.pt.x, kp_y = (int)kp.pt.y;

        if (kp_x < 8 || kp_y < 8 || kp_x > image.cols - 8 || kp_y > image.rows - 8) {
            continue;
        }

        for (int i = -half_patch_size; i < half_patch_size; i++) {
            for (int j = -half_patch_size; j < half_patch_size; j++) {
                weighted_x += image.at<uchar>(kp_y + i, kp_x + j) * j;
                weighted_y += image.at<uchar>(kp_y + i, kp_x + j) * i;
            }
        }

        kp.angle = (180 / pi) * atan2(weighted_y, weighted_x);
        if (kp.angle < 0) {
            kp.angle += 360;
        }
        // END YOUR CODE HERE
...
void computeORBDesc(const cv::Mat &image, vector<cv::KeyPoint> &keypoints, vector<DescType> &desc) {
    for (auto &kp: keypoints) {
        ...
        double sin_theta = std::sin(kp.angle * pi / 180), cos_theta = std::cos(kp.angle * pi / 180);
        for (int i = 0; i < 256; i++) {
            // START YOUR CODE HERE (~7 lines)
            int px = ORB_pattern[i * 4], py = ORB_pattern[i * 4 + 1], qx = ORB_pattern[i * 4 + 2], qy = ORB_pattern[i * 4 + 3];
            double rot_px  = cos_theta * px - sin_theta * py,
                         rot_py = sin_theta * px + cos_theta * py,
                         rot_qx  = cos_theta * qx - sin_theta * qy,
                         rot_qy = sin_theta * qx + cos_theta * qy;

            if (kp.pt.x +rot_px < 0 || kp.pt.x +rot_px > image.cols || kp.pt.y + rot_py < 0 || kp.pt.y + rot_py > image.rows
                || kp.pt.x +rot_qx < 0 || kp.pt.x +rot_qx > image.cols || kp.pt.y + rot_qy < 0 || kp.pt.y + rot_qy > image.rows
                || kp.pt.x < 8 || kp.pt.y < 8 || kp.pt.x > image.cols - 8 || kp.pt.y > image.rows - 8) {
                d.clear();
                break;
            }

            d[i] = image.at<uchar>((int)(kp.pt.y + rot_py), (int)(kp.pt.x + rot_px)) > image.at<uchar>((int)(kp.pt.y + rot_qy), (int)(kp.pt.x + rot_qx)) ? 0 : 1;  // if kp goes outside, set d.clear()
	    // END YOUR CODE HERE
        }
        desc.push_back(d);
    }
...
void bfMatch(const vector<DescType> &desc1, const vector<DescType> &desc2, vector<cv::DMatch> &matches) {
    ...
    for (int queryIdx = 0; queryIdx < desc1.size(); queryIdx++) {
        auto current_query_desc = desc1.at(queryIdx);
        int  min_distance = 500;

        if (current_query_desc.empty()) continue;
        cv::DMatch m;

        for (int trainIdx = 0; trainIdx < desc2.size(); trainIdx++) {
            int distance = 0;
            auto current_train_desc = desc2.at(trainIdx);
            if (current_train_desc.empty()) continue;

            for (int i = 0; i < 256; i++) {
                if (current_query_desc.at(i) != current_train_desc.at(i)) {
                    distance += 1;
                }
            }
